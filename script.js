...
function renderSections(data) {
  ...
  data.forEach(item => {
    const sectionId = item.type.trim().replace(/\s+/g, '-').toLowerCase();
  ...
  // Créer et ajouter le bouton "Tout afficher"
  const allBtn = document.createElement('button');
  allBtn.textContent = 'Tout afficher';
  allBtn.addEventListener('click', () => {
    const firstSectionId = document.querySelector('h2')?.id;
    scrollToSection(firstSectionId);
  });
  nav.appendChild(allBtn);

  // Créer les boutons de section et le contenu
  sections.forEach(section => {
    // Créer le bouton
    ...
    title.id = section.id;
    title.classList.add('visible'); // visible par défaut
    container.appendChild(title);

    // Créer la section
    const sectionContainer = document.createElement('div');
    sectionContainer.classList.add('section-container', 'visible'); // visible par défaut
